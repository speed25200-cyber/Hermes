// GROSSES CAPS — PUMP (memecoin le plus liquide, >100 M$/j) : extrême RSI14-15m (<30 / >70)
// suivi d'une bougie 15m de retournement -> fade des deux côtés, TP 0.80, trail 0.20/0.05, 12 h.
module.exports = {
  ...
  detect(c5) {
    ...
    for (let i = 16; i < bars.length; i++) {
      if (r[i - 1] < 30 && bars[i][4] > bars[i][1]) out.push({ i5: i5last[i], dir: 1 });
      else if (r[i - 1] > 70 && bars[i][4] < bars[i][1]) out.push({ i5: i5last[i], dir: -1 });
    }
    return out;
  }
};
