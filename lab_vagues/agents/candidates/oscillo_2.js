// CCI(20) sur 15m en zone extrême (±200) : excès violent que l'on fade immédiatement (mean-reversion).
module.exports = {
  ...
  detect(c5) {
    ...
    for (let j = 60; j < bars.length; j++) {
      if (cci[j] === null) continue;
      if (cci[j] < -200) out.push({ i5: i5last[j], dir: 1 });
      else if (cci[j] > 200) out.push({ i5: i5last[j], dir: -1 });
    }
    return out;
  }
};
