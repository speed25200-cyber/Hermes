...
// signal = SMA(macd,9) — PAS ema, comme le script original (le point souvent oublié en le portant).
// Lecture (c) : le MACD 1h sert de FILTRE DE CONTRE-TENDANCE sur un déclencheur RSI(7) 5m qui "reclaim"
// une zone extrême (25/75) — on ne prend le rebond 5m QUE quand il va à CONTRE-SENS du MACD 1h établi
// (acheter un creux RSI si le 1h est baissier, vendre un sommet RSI si le 1h est haussier) : fade
// mécanique de l'excès local dans un régime 1h déjà tendanciel, pas un suivi de tendance.
module.exports = {
  instId: "KGEN-USDT-SWAP",
  exits: { tp: 0.80, sl: 0.30, act: 0.30, cb: 0.05, holdH: 12 },
  detect(c5) {
    const RSI_N = 7, LO = 25, HI = 75;

    // --- agrégation 1h calendaire (12 bougies 5m), i5last = index 5m de la DERNIÈRE bougie du bloc (close connu) ---
    ...
    // --- EMA(n) standard (seed = SMA des n premières valeurs) ---
    function ema(vals, n) {
    ...
    // --- SMA(n) glissante sur série pouvant démarrer par des null ---
    ...
    // MACD Pine : fastLength=12, slowLength=26, signalLength=9, signal = SMA(macd,9)
    ...
    // état 1h (macd_IsAbove = macd >= signal) reporté sur chaque bougie 5m, connu SEULEMENT à partir
    // de la clôture de l'heure qui l'a produit (pas de repaint) et jusqu'à la clôture de la suivante
    ...
    for (let i = RSI_N + 2; i < c5.length; i++) {
      ...
      const longTrig = rsi[i - 1] < LO && rsi[i] >= LO;   // reclaim depuis survente
      const shortTrig = rsi[i - 1] > HI && rsi[i] <= HI;  // reclaim depuis surachat
      const want = -state[i]; // CONTRA : on prend le trade à contre-sens du MACD 1h
