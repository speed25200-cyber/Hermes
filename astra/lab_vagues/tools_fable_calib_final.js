// ÉVALUATION FINALE CALIBRAGE (directeur calibrage, itération 1) — FORMULE FIGÉE avant
// toute lecture OOS/epoque2 (discipline et amendements : tools_fable_calib_NOTES.md).
//
// FORMULE "CALIB-1" :
...
//   + filtre 3 (étape B) : ret1h CONTRE le trade (achat si close_i/close_{i-12}-1 <= 0, vente si >= 0)
//   + filtre 4 (étape C2) : volume per-crypto — rang causal du volSpike parmi les signaux
//     PRÉCÉDENTS du même instrument >= p70 (fenêtre expansive ; inactif tant que < 20 obs)
...
//
// Une commande : node tools_fable_calib_final.js
...
const dec = (d, s, f) => {
  const nd = -d;
  const volOK = f.volSpike >= 2.5 && (f.volRankPrev < 0 || f.volRankPrev >= 0.7);
  return volOK && acc(nd, f.rangePos) && contre(nd, f) ? nd : 0;
};
const CANDIDAT = { nom: "FORMULE CALIB-1", lockHold: true, ex: EXC, exKey: "EXC", decide: dec };
...
const variantes = [
  ...
  { nom: "réf boucle 2 (V2)", lockHold: true, ex: { tp: 0.8, sl: 0.3, act: 0.25, cb: 0.15, holdH: 12 }, exKey: "EXF", decide: (d, s, f) => { const nd = -d; return f.volSpike >= 2.5 && acc(nd, f.rangePos) ? nd : 0; } },
];
const res = evaluer(corpus, midAout, variantes);
console.log("\n========== FORMULE CALIB-1 (tp80/sl30/act20/cb15/hold12 · verrou 12h · levier 15 · coûts 0,12 % · banc3 sans look-ahead) ==========");
console.log("INV s>=2 · vol >= 2,5x méd24h ET rang per-crypto >= p70 (causal) · accord range moitiés · ret1h CONTRE · entrée open suivante\n");
...
console.log("\ncôtés + référence boucle 2 :");
for (const k of [1, 2, 3])
  console.log("  " + variantes[k].nom.padEnd(20), ["aout_IS", "aout_OOS", "epoque2"].map(e => fmt(agg(res[k][e]))).join(" | "));
