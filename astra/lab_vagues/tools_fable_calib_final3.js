// ÉVALUATION FINALE CALIBRAGE — ITÉRATION 3 (directeur calibrage) — FORMULE FIGÉE avant
// toute lecture OOS/epoque2 (discipline : tools_fable_calib_NOTES.md, gel A7).
//
// FORMULE "CALIB-3" (= CALIB-2 MOINS volRank, cause identifiée du point faible OOS) :
...
//   + filtre 3 (unique filtre empilé) : ret1h CONTRE le trade
//     (achat si close_i/close_{i-12}-1 <= 0, vente si >= 0)
...
//
// Une commande : node tools_fable_calib_final3.js
...
const dec = (d, s, f) => {
  const nd = -d;
  return s < 2.5 && f.volSpike >= 2.5 && acc(nd, f.rangePos) && contre(nd, f) ? nd : 0;
};
const CANDIDAT = { nom: "FORMULE CALIB-3", lockHold: true, ex: EXC, exKey: "EXC", decide: dec };
...
// rangeRank per-crypto causal — uniquement pour la ligne de CONSTAT léguée (cellule 4 d'A3)
...
const accRank = (nd, f, p) => f.rangeRankPrev < 0 ? acc(nd, f.rangePos) : ((nd > 0 && f.rangeRankPrev <= p) || (nd < 0 && f.rangeRankPrev >= 1 - p));
// volRank per-crypto causal — uniquement pour la ligne de CONSTAT CALIB-2
for (const inst of Object.values(corpus)) {
  const prev = [];
  for (const s of inst.sigs) {
...
const variantes = [
  ...
  // lignes de CONSTAT (référence, non sélectionnées) :
  { nom: "réf ret1h seul (sans cap)", lockHold: true, ex: EXC, exKey: "EXC", decide: (d, s, f) => { const nd = -d; return f.volSpike >= 2.5 && acc(nd, f.rangePos) && contre(nd, f) ? nd : 0; } },
  { nom: "réf CALIB-2 (avec volRank p70)", lockHold: true, ex: EXC, exKey: "EXC", decide: (d, s, f) => { const nd = -d; return s < 2.5 && f.volSpike >= 2.5 && (f.volRankPrev < 0 || f.volRankPrev >= 0.7) && acc(nd, f.rangePos) && contre(nd, f) ? nd : 0; } },
  { nom: "legs boucle4: ret1h+rangeRank p50", lockHold: true, ex: EXC, exKey: "EXC", decide: (d, s, f) => { const nd = -d; return f.volSpike >= 2.5 && accRank(nd, f, 0.5) && contre(nd, f) ? nd : 0; } },
];
const res = evaluer(corpus, midAout, variantes);
console.log("\n========== FORMULE CALIB-3 (tp80/sl30/act20/cb15/hold12 · verrou 12h · levier 15 · coûts 0,12 % · banc3 sans look-ahead) ==========");
console.log("INV 2<=s<2.5 · vol >= 2,5x méd24h · accord range moitiés · ret1h CONTRE (unique filtre empilé) · entrée open suivante\n");
