// VARIANTE 2 : label des voisins avec zone morte (epsilon) au lieu du simple signe — filtre le bruit
// proche de zéro (< coûts) avant le vote, cf. tv2_knn_scan.js pour la variante de base.
...
const EPS = 0.0015;         // zone morte du label voisin (~coût A/R), variante 2 seulement
...
function computeVotes(feats, closes, n, distType) {
  ...
  for (let i = START; i < n; i++) {
    ...
    for (let j = jMin; j <= jMax; j++) {
      ...
      const ret = closes[j + HORIZON] / closes[j] - 1;
      const lbl = ret > EPS ? 1 : (ret < -EPS ? -1 : 0);
      if (cnt < K) {
        topDist[cnt] = d;
        topLabel[cnt] = lbl;
        cnt++;
...
for (const instId of univers_ids) {
  ...
  for (const distType of DIST_TYPES) {
    ...
    for (const T of THRESHOLDS) {
      const sigs = [];
      for (let i = START; i < n; i++) {
        if (votes[i] >= T) sigs.push({ i5: i, dir: 1 });
        else if (votes[i] <= -T) sigs.push({ i5: i, dir: -1 });
      }
      ...
      for (const [exitName, ex] of Object.entries(EXITS)) {
        ...
        if (worst < 3) continue;
        results.push({
          instId, distType, T, exit: exitName,
...
fs.writeFileSync(path.join(__dirname, "rapports", "tv2_knn_scan2_resultats.json"), JSON.stringify(results, null, 1));
