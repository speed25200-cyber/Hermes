// MUR — ÉLASTICITÉ PRIX-VOLUME (invention) — PEOPLE (ConstitutionDAO), lecture PERCÉE.
...
// Lecture (2 phrases) : CR <= 0,7 avec volume de fenêtre >= la moyenne 24 h = prix figé sous
// beaucoup de volume. Sur PEOPLE le mur ne tient pas : si un mouvement fort (>= 2 x bruit x
// sqrt(24)) pousse dessus, c'est de l'accumulation avant PERCÉE -> on SUIT le mouvement.
const W = 6;        // fenêtre du mur (30 min)
const R = 0.7;      // seuil d'effondrement de l'élasticité
...
module.exports = {
  ...
  detect(c5) {
    ...
    for (let i = WARM; i < c5.length - 2; i++) {
      ...
      if (!(Math.abs(app) >= th) || th <= 0) continue;  // un mouvement fort pousse dessus
      out.push({ i5: i, dir: app > 0 ? 1 : -1 });       // accumulation avant percée : continuation
    }
    return out;
  }
};
