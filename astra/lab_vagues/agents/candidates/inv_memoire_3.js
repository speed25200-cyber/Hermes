// MNIV — MÉMOIRE DES NIVEAUX (invention) — GRAM, lecture « any » (mémoire totale), contact serré
// Carte causale des niveaux : tranches log de 0,25 %. Chaque pivot fractal confirmé (aile 6 bougies,
// confirmé 30 min après l'extrême) dépose w = min(vol/moyVol96, 4) dans sa tranche et ses 2 voisines
// (demi-poids) ; mémoire décayée en demi-vie 48 h → poids du niveau = pivots × récence × volume.
// Sur GRAM la mémoire TOTALE (pivots hauts + bas confondus : un niveau très travaillé agit dans les
// deux rôles) bat la lecture par côté.
// Lecture (2 phrases) : quand une impulsion 15 min (>= 2× le bruit moyen) amène le prix au contact
// serré (<= 0,3 %) d'un niveau à mémoire totale >= 3 pivots efficaces dont il était éloigné 30 min
// avant, ET que la vitesse s'éteint à l'approche (|15 min courantes| <= 50 % des précédentes),
// le niveau se rappelle au marché : on fade l'approche. Zéro look-ahead.
const L = 6;          // aile du pivot fractal (30 min)
const HALF = 48;      // demi-vie de la mémoire (heures)
const M = 3;          // seuil de mémoire TOTALE (pivots efficaces)
const D = 0.003;      // distance de contact au niveau (log)
const DEC = 0.5;      // extinction de vitesse exigée
const MINK = 2;       // approche réelle : |v 15 min| >= 2 × bruit moyen par bougie
