// MNIV — MÉMOIRE DES NIVEAUX (invention) — ORDI, lecture « matched », tempo 1 h / mémoire 24 h
// Carte causale des niveaux : tranches log de 0,25 %. Chaque pivot fractal confirmé (aile 12 bougies
// = 1 h, confirmé 1 h après l'extrême) dépose w = min(vol/moyVol96, 4) dans sa tranche et ses 2
// voisines (demi-poids) ; mémoire décayée en demi-vie 24 h → poids du niveau = pivots × récence × volume.
// Côtés séparés : mémoire-support (pivots BAS) / mémoire-résistance (pivots HAUTS).
// Lecture (2 phrases) : quand une vraie impulsion 15 min (>= 3× le bruit moyen) ramène le prix au
// contact (<= 0,6 %) d'un niveau dont la mémoire DANS CE RÔLE (support si chute, résistance si montée)
// vaut >= 1,5 pivots efficaces, niveau quitté depuis >= 30 min, ET que la vitesse s'éteint à l'approche
// (|15 min courantes| <= 50 % des précédentes) → le niveau tient : on fade l'approche. Zéro look-ahead.
const L = 12;         // aile du pivot fractal (1 h)
