...
async function rest(method,path,body){assertLegacyOrderIsReduceOnly(method,path,body); const ts=new Date().toISOString(); const key=process.env.OKX_API_KEY,sec=process.env.OKX_API_SECRET,pp=process.env.OKX_API_PASSPHRASE;
...
class OkxTrader{
  ...
  async orderWS(args){assertLegacyOrderIsReduceOnly('POST','/api/v5/trade/order',args); await this.connectPrivate(); const payload=JSON.stringify({op:'order',args:[args]}); if(this.wsReady)this.ws.send(payload); else this.queue.push(payload); return { via:'ws', sent:true, args }; }
  ...
  async placeMarketWithAttach({instId,side,tdMode='isolated',posSide,sz,tpPx,slPx,tpTrigType='mark',slTrigType='mark'}){
    void instId; void side; void tdMode; void posSide; void sz; void tpPx; void slPx; void tpTrigType; void slTrigType;
    const error=new Error('LEGACY_REAL_ENTRY_DISABLED_USE_APP_MAIN_GATED_EXECUTOR');error.code='LEGACY_ENTRY_DISABLED';throw error;
  }
  async placeTrailing({instId,side,tdMode='isolated',callbackRatio='0.003',activePx}){
    const args={instId,tdMode,side,ordType:'move_order_stop',callbackRatio:String(callbackRatio),activePx:String(activePx),reduceOnly:true};
    try{ return await this.orderWS(args);}catch(e){ return await rest('POST','/api/v5/trade/order-algo',args); }
  }
}
...
module.exports={OkxTrader,computeSz,calcPx,rest,assertLegacyOrderIsReduceOnly};
